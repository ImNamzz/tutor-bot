...
export default function Home() {
  ...
  const generateSessionName = async (content: string): Promise<string> => {
    try {
      ...
      const response = await fetch(API_ENDPOINTS.chat, {
        ...
        body: JSON.stringify({
          session_id: null,
          message: `Generate a very short, concise title (3-5 words max) for a chat session based on this content: "${content.substring(
            0,
            200
  ...
  const handleFileUpload = async (
    ...
    if (fileExtension === ".doc" || fileExtension === ".docx") {
      try {
        ...
        generateSessionName(data.transcript || file.name).then(
          (generatedName) => {
  ...
  const handleDeleteSession = (e: React.MouseEvent, sessionId: string) => {
    ...
    const filtered = savedSessions.filter(
      (s: TranscriptSession) => s.id !== sessionId
    );

...
export default function Home() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    addMessage('user', `≡ƒôÄ Uploaded: ${file.name}`)
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    addMessage('user', `≡ƒû╝∩╕Å Uploaded image: ${file.name}`)
  ...
  const handleDeleteSession = (e: React.MouseEvent, sessionId: string) => {
    ...
    if (sessionId === currentSessionId) {
      ...
      addMessage('assistant', 
        "Your previous session has been deleted. Feel free to start a new conversation or upload a file! ≡ƒôÜ"
      )
    }
  ...
  const handleReset = () => {
    ...
    addMessage('assistant', 
      "Welcome back! Upload a new lecture transcript to start a fresh learning session. ≡ƒôÜ"
    )
  }
